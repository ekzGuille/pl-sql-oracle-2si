...
AS
    CURSOR V_LISTA IS 
        SELECT ARTICULO, PRECIO_VENTA, EXISTENCIAS
        FROM ARTICULOS 
...
BEGIN 
    OPEN V_LISTA;
    FETCH V_LISTA INTO V_ARTICULO, V_PRECIO_VENTA, V_EXISTENCIAS;

    IF V_LISTA%NOTFOUND THEN
        DBMS_OUTPUT.PUT_LINE('NO SE ENCUENTRAN LOS DATOS');
    ELSE 
        WHILE V_LISTA%FOUND LOOP
            DBMS_OUTPUT.PUT_LINE(V_ARTICULO || ' - ' || V_PRECIO_VENTA || '€ - ' ||  V_EXISTENCIAS);
            FETCH V_LISTA INTO V_ARTICULO, V_PRECIO_VENTA, V_EXISTENCIAS;
        END LOOP;
    END IF;
    CLOSE V_LISTA;
