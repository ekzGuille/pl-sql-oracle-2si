...
BEGIN 
  IF (:OLD.UNIDADES < 6) THEN
      INSERT INTO tabla_log_borrado VALUES ('ERROR AL ELIMINAR REGISTRO CON ID ' || :OLD.COD_LIBRO || ' A LAS ' || TO_CHAR(SYSDATE, 'DD-MM-YYY HH24:MI:SS'));
      RAISE e_trigger;
  END IF;
...
BEGIN
  IF(:NEW.DEPT_NO = 10) THEN
    RAISE cantidad_error;
  END IF;
  INSERT INTO tabla_log_borrado VALUES ('INSERTADO REGISTRO CON ID ' || :OLD.COD_LIBRO || ' A LAS ' || TO_CHAR(SYSDATE, 'DD-MM-YYY HH24:MI:SS'));

  EXCEPTION cantidad_error THEN
