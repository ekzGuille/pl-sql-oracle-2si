...
CREATE OR REPLACE PROCEDURE INSERT_BANCOS(banco BANCOS_OBJ)
  ...
  EXCEPTION 
    ...
    WHEN OTHERS THEN
      RAISE_APPLICATION_ERROR(-20001, 'HA HABIDO UN ERROR: - ' || SQLCODE || ' -ERROR- ' || SQLERRM); 
  ...
  BEFORE INSERT OR UPDATE OR DELETE
    ...
    EXCEPTION 
      WHEN NO_DATA_FOUND THEN
        RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO INTRODUCIDO NO EXISTE');
      WHEN e_PK_COD_BANCO_EXISTE THEN
        RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO INTRODUCIDO YA EXISTE');
      WHEN e_PK_COD_BANCO_MODIFICAR THEN
        RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO NO SE PUEDE MODIFICAR');
      WHEN e_POBLACION_NO_EXISTE THEN
        RAISE_APPLICATION_ERROR(-99999,'HA DE INTRODUCIR UNA POBLACION DE LAS EXISTENTES');
END;
/
  ...
  BEGIN
    ...
    IF DELETING THEN 
      ...
      IF v_cod_banco_sucur = 0 THEN
        RAISE NO_DATA_FOUND;
      ELSE
        DELETE FROM CUENTAS WHERE COD_BANCO = :NEW.COD_BANCO AND COD_SUCUR = :NEW.COD_SUCUR;
      END IF;
    END IF;

  EXCEPTION 
    WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO Y/O COD_SUCUR INTRODUCIDO NO EXISTE');
    WHEN e_PK_COD_BANCO_SUCUR_EXISTE THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO Y/O COD_SUCUR INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
    WHEN e_PK_COD_BANCO_SUCUR_MOD THEN
    RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO Y/O COD_SUCUR NO SE PUEDEN MODIFICAR');
    WHEN e_LOCALIDAD_NO_EXISTE THEN
        RAISE_APPLICATION_ERROR(-99999,'HA DE INTRODUCIR UNA LOCALIDAD DE LAS EXISTENTES');
END;
/
...
CREATE OR REPLACE TRIGGER CUENTAS_BEFORE_TR
  ...
  DECLARE
    ...
    v_cod_sucur NUMBER;
    v_num_cuenta NUMBER;
    v_saldo_debe CUENTAS.SALDO_DEBE%TYPE;
    v_saldo_haber CUENTAS.SALDO_HABER%TYPE;
  ...
  BEGIN
    ...
    IF DELETING THEN 
      ...
      IF v_cod_banco > 0 THEN
        SELECT SALDO_DEBE, SALDO_HABER INTO v_saldo_debe, v_saldo_haber FROM CUENTAS WHERE COD_BANCO = :OLD.COD_BANCO AND COD_SUCUR = :OLD.COD_SUCUR AND NUM_CTA = :OLD.NUM_CTA;
        IF v_saldo_debe > 0 OR v_saldo_haber > 0 THEN
  ...
  EXCEPTION 
    ...
    WHEN e_PK_COD_BAN_SUCUR_NCTA_EXISTE THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
    WHEN e_PK_COD_BAN_SUCUR_NCTA_MOD THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO EXISTE, NO SE PUEDEN MODIFICAR');
    WHEN e_DELETE_SIN_SALDO_0 THEN
      RAISE_APPLICATION_ERROR(-99999,'NO PUEDE BORRAR UNA CUENTA CON SALDO MAYOR QUE 0');
END;
/
...
CREATE OR REPLACE TRIGGER MOVIMIENTOS_BEFORE_TR
  ...
  EXCEPTION 
    WHEN NO_DATA_FOUND THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO NO EXISTE');
    WHEN e_PK_COD_BAN_SUCUR_NCTA_EXISTE THEN
      RAISE_APPLICATION_ERROR(-99999,'EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
    WHEN e_MOV_FUTURO THEN 
      RAISE_APPLICATION_ERROR(-99999,'NO SE PUEDEN REALIZAR MOVIMIENTOS A FUTURO');
    WHEN e_POBLACION_DISTINTA THEN 
      RAISE_APPLICATION_ERROR(-99999,'NO SE HACER MOVIMIENTOS A CUENTAS CON POBLACION DISTINTA A LA EXISTENTE');
END;
/
