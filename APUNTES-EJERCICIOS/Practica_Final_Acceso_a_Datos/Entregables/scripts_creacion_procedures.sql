...
CREATE OR REPLACE TRIGGER BANCOS_BEFORE_TR 
  BEFORE INSERT OR UPDATE OR DELETE
    ...
    BEGIN
      IF INSERTING THEN
        SELECT COD_BANCO INTO v_cod_banco FROM BANCOS WHERE COD_BANCO = :NEW.COD_BANCO;
      ...
      IF DELETING THEN
        SELECT COD_BANCO INTO v_cod_banco FROM BANCOS WHERE COD_BANCO = :OLD.COD_BANCO;
      END IF;
...
CREATE OR REPLACE TRIGGER SUCURSALES_BEFORE_TR
  ...
  DECLARE
    v_cod_banco BANCOS.COD_BANCO%TYPE;
    v_cod_sucur SUCURSALES.COD_SUCUR%TYPE;
    e_PK_COD_BANCO_EXISTE EXCEPTION;
  ...
  BEGIN
    IF INSERTING THEN
      ...
      SELECT COD_BANCO, COD_SUCUR INTO v_cod_banco, v_cod_sucur FROM SUCURSALES WHERE COD_BANCO = :NEW.COD_BANCO AND COD_SUCUR = :NEW.COD_SUCUR;
      IF SQL%ROWCOUNT = 1 THEN
    ...
    IF DELETING THEN 
      SELECT COD_BANCO, COD_SUCUR INTO v_cod_banco, v_cod_sucur FROM SUCURSALES WHERE COD_BANCO = :OLD.COD_BANCO AND COD_SUCUR = :OLD.COD_SUCUR;
    END IF;
  ...
  EXCEPTION 
    ...
    WHEN e_PK_COD_BANCO_EXISTE THEN
      DBMS_OUTPUT.PUT_LINE('EL COD_BANCO INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
    WHEN e_PK_COD_BANCO_SUCUR_EXISTE THEN
      DBMS_OUTPUT.PUT_LINE('EL COD_BANCO Y/O COD_SUCUR INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
...
CREATE OR REPLACE TRIGGER CUENTAS_BEFORE_TR
  ...
  DECLARE
    v_cod_banco BANCOS.COD_BANCO%TYPE;
    v_cod_sucur SUCURSALES.COD_SUCUR%TYPE;
    v_num_cuenta CUENTAS.NUM_CTA%TYPE;
  ...
  BEGIN
    IF INSERTING THEN
      ...
      SELECT COD_BANCO, COD_SUCUR, NUM_CTA INTO v_cod_banco, v_cod_sucur, v_num_cuenta FROM CUENTAS WHERE COD_BANCO = :NEW.COD_BANCO AND COD_SUCUR = :NEW.COD_SUCUR AND NUM_CTA = :NEW.NUM_CTA;
      IF SQL%ROWCOUNT = 1 THEN
    ...
    IF DELETING THEN 
      SELECT COD_BANCO, COD_SUCUR, NUM_CTA INTO v_cod_banco, v_cod_sucur, v_num_cuenta FROM CUENTAS WHERE COD_BANCO = :OLD.COD_BANCO AND COD_SUCUR = :OLD.COD_SUCUR AND NUM_CTA = :OLD.NUM_CTA;
  ...
  EXCEPTION 
    ...
    WHEN e_PK_COD_BAN_SUCUR_NCTA_EXISTE THEN
      DBMS_OUTPUT.PUT_LINE('EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO EXISTE, NO SE PUEDEN SOBRESCRIBIR');
    WHEN e_PK_COD_BAN_SUCUR_NCTA_MOD THEN
      DBMS_OUTPUT.PUT_LINE('EL COD_BANCO, COD_SUCUR Y/O NUM_CTA INTRODUCIDO EXISTE, NO SE PUEDEN MODIFICAR');
    WHEN e_PK_COD_BANCO_SUCUR_MOD THEN
    DBMS_OUTPUT.PUT_LINE('EL COD_BANCO Y/O COD_SUCUR NO SE PUEDEN MODIFICAR');
    WHEN e_DELETE_SIN_SALDO_0 THEN
    DBMS_OUTPUT.PUT_LINE('NO PUEDE BORRAR UNA CUENTA CON SALDO MAYOR QUE 0');
END;
/
